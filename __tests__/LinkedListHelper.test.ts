...
describe("LinkedListHelper", () => {
  ...
  test("toArray should return [1,2]", () => {
    expect(
      LinkedListHelper.prototype.convertToArray({
        value: 1,
  ...
  test("should return ListNode instance", () => {
    ...
    expect(LinkedListHelper.prototype.getNode(new ListNode(1))).toBeInstanceOf(
      ListNode
    );
  });
  ...
  test("should remove cycle from list", () => {
    ...
    LinkedListHelper.prototype.removeCycle(
      dummyList.cycleAtNode,
      dummyList.head
    );
